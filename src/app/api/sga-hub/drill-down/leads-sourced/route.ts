// src/app/api/sga-hub/drill-down/leads-sourced/route.ts
...
export async function GET(request: NextRequest) {
  try {
    ...
    const records = await getLeadsSourcedDrillDown(sgaName, weekStartDate, weekEndDate, selfSourcedOnly);

    return NextResponse.json({ records });
  } catch (error) {
    console.error('Error fetching leads sourced drill-down:', error);
    return NextResponse.json(
      { error: 'Failed to fetch leads sourced records' },
      { status: 500 }
    );
  }
}
