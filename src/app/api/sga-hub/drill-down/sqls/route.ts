// src/app/api/sga-hub/drill-down/sqls/route.ts
...
export async function GET(request: NextRequest) {
  try {
    ...
    const records = await getSQLDrillDown(sgaName, weekStartDate, weekEndDate);

    return NextResponse.json({ records });
  } catch (error) {
    console.error('Error fetching SQL drill-down:', error);
    return NextResponse.json(
      { error: 'Failed to fetch SQL records' },
      { status: 500 }
    );
  }
}
