...
import { getSGMQuotaProgress } from '@/lib/queries/sgm-quota';
...
export async function GET(request: NextRequest) {
  try {
    ...
    const progress = await getSGMQuotaProgress(sgmName, quarter);
    return NextResponse.json({ progress });
  } catch (error) {
    console.error('[API] Error fetching SGM quota progress:', error);
    return NextResponse.json(
      { error: 'Failed to fetch quota progress' },
      { status: 500 }
    );
  }
}
