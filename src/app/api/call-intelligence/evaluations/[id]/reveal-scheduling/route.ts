...
import { RevealSchedulingRequest } from '@/lib/sales-coaching-client/schemas';
...
export async function PATCH(request: NextRequest, ctx: { params: Promise<{ id: string }> }) {
  try {
    ...
    const parsed = RevealSchedulingRequest.safeParse(json);
    if (!parsed.success) return NextResponse.json({ error: 'Invalid body', issues: parsed.error.issues }, { status: 400 });

    const result = await salesCoachingClient.setRevealScheduling(session.user.email, id, parsed.data);
    ...
    console.error('[/api/call-intelligence/evaluations/[id]/reveal-scheduling] error', err);
