...
import { SubmitNoteReviewRequest } from '@/lib/sales-coaching-client/schemas';
...
export async function POST(request: NextRequest, ctx: { params: Promise<{ callNoteId: string }> }) {
  try {
    ...
    const parsed = SubmitNoteReviewRequest.safeParse(json);
    ...
    console.error('[note-review submit] error', err);
