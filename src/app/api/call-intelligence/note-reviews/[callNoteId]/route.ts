...
import { EditCallNoteRequest } from '@/lib/sales-coaching-client/schemas';

export const dynamic = 'force-dynamic';
const UUID_RE = /^[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12}$/i;

export async function GET(_req: NextRequest, ctx: { params: Promise<{ callNoteId: string }> }) {
  try {
    ...
    const result = await salesCoachingClient.getCallNoteReview(session.user.email, callNoteId);
    ...
    console.error('[note-review get] error', err);
...
export async function PATCH(request: NextRequest, ctx: { params: Promise<{ callNoteId: string }> }) {
  try {
    ...
    const parsed = EditCallNoteRequest.safeParse(json);
    ...
    if (err instanceof BridgeAuthError) return NextResponse.json({ error: err.message, requestId: err.requestId }, { status: err.status });
    if (err instanceof BridgeTransportError) return NextResponse.json({ error: err.message, requestId: err.requestId }, { status: err.status || 500 });
    console.error('[note-review edit] error', err);
    return NextResponse.json({ error: err instanceof Error ? err.message : 'Failed' }, { status: 500 });
  }
}
