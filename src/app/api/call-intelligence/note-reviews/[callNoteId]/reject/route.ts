...
import { RejectNoteReviewRequest } from '@/lib/sales-coaching-client/schemas';

export const dynamic = 'force-dynamic';
const UUID_RE = /^[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12}$/i;

export async function POST(request: NextRequest, ctx: { params: Promise<{ callNoteId: string }> }) {
  try {
    ...
    const parsed = RejectNoteReviewRequest.safeParse(json);
    if (!parsed.success) return NextResponse.json({ error: 'Invalid body', issues: parsed.error.issues }, { status: 400 });
    const result = await salesCoachingClient.rejectNoteReview(session.user.email, callNoteId, parsed.data);
    ...
    console.error('[note-review reject] error', err);
