...
import { SfdcSearchRequest } from '@/lib/sales-coaching-client/schemas';
...
export async function POST(request: NextRequest, ctx: { params: Promise<{ callNoteId: string }> }) {
  try {
    ...
    const parsed = SfdcSearchRequest.safeParse(json);
    if (!parsed.success) return NextResponse.json({ error: 'Invalid body', issues: parsed.error.issues }, { status: 400 });
    const result = await salesCoachingClient.searchSfdcForNote(session.user.email, callNoteId, parsed.data);
    ...
    console.error('[note-review sfdc-search] error', err);
