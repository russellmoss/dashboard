...
import { ContentRefinementResolveRequest } from '@/lib/sales-coaching-client/schemas';
...
export async function POST(request: NextRequest, ctx: { params: Promise<{ id: string }> }) {
  try {
    ...
    const parsed = ContentRefinementResolveRequest.safeParse(json);
    if (!parsed.success) return NextResponse.json({ error: 'Invalid body', issues: parsed.error.issues }, { status: 400 });
    const result = await salesCoachingClient.resolveContentRefinement(session.user.email, id, parsed.data);
    revalidateTag(CACHE_TAGS.CALL_INTELLIGENCE_QUEUE);
    return NextResponse.json(result);
  } catch (err) {
    if (err instanceof ContentRefinementAlreadyResolvedError) {
      return NextResponse.json({
        error: 'Content refinement was already resolved by another admin — refresh',
    ...
    console.error('[/api/call-intelligence/refinements/[id]/resolve] error', err);
