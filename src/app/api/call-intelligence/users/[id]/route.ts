...
import { UpdateUserRequest } from '@/lib/sales-coaching-client/schemas';
...
export async function PATCH(request: NextRequest, ctx: { params: Promise<{ id: string }> }) {
  try {
    ...
    const parsed = UpdateUserRequest.safeParse(json);
    if (!parsed.success) return NextResponse.json({ error: 'Invalid body', issues: parsed.error.issues }, { status: 400 });
    const result = await salesCoachingClient.updateUser(session.user.email, id, parsed.data);
    ...
    console.error('[/api/call-intelligence/users/[id] PATCH] error', err);
