...
import { BulkReassignRequest } from '@/lib/sales-coaching-client/schemas';
...
export async function POST(request: NextRequest, ctx: { params: Promise<{ id: string }> }) {
  try {
    ...
    const parsed = BulkReassignRequest.safeParse(json);
    if (!parsed.success) return NextResponse.json({ error: 'Invalid body', issues: parsed.error.issues }, { status: 400 });
    const result = await salesCoachingClient.bulkReassignPendingEvals(session.user.email, id, parsed.data);
    ...
    console.error('[/api/call-intelligence/users/[id]/bulk-reassign-pending-evals] error', err);
