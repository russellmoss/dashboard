...
interface SGMLeaderboardFiltersProps {
...
export function SGMLeaderboardFilters({
  ...
  }, [initialQuarter, initialChannels, initialSources, initialSGMs]);
  ...
  const handleSelectAllSgms = () => {
    if (disabled || sgmOptionsLoading) return;
  ...
  // Reset all filters to defaults — SGM Hub defaults to ALL channels
  const handleResetFilters = () => {
    if (disabled) return;
    const currentQuarter = getCurrentQuarter();
    setLocalQuarter(currentQuarter);
    setLocalChannels([...channelOptions]); // ALL channels (differs from SGA)
    setLocalSources([...sourceOptions]);
  ...
  const sgmsSummary = sgmOptionsLoading
    ? 'Loading...'
    : initialSGMs.length === sgmOptions.filter(s => s.isActive).length
      ? 'All Active SGMs'
  ...
  // SGM Hub: custom = anything other than all channels
  const hasCustomFilters =
    initialChannels.length !== channelOptions.length ||
