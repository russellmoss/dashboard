...
**Implementation**: `src/lib/permissions.ts` defines `ROLE_PERMISSIONS` with base permissions, then adds role-specific filters based on user's role and name/agency. Note: `sgmFilter` is always `null` (SGM scoping was removed — SGMs have full visibility like admins).
...
Some API routes enforce `sgaFilter` by overriding the frontend-supplied filter with the user's own name (SGM scoping was removed — SGMs now have full visibility like admins):
...
SGMs have full visibility across all SGM data (no `sgmFilter` scoping). Components:
...
- `src/app/dashboard/call-intelligence/evaluations/[id]/page.tsx` — eval drill-down sub-route. AI Evaluation panel starts collapsed (click to expand dimension scores). Manager/admin roles see per-section "Flag" buttons to report incorrect AI content; flags write to `ai_feedback` via bridge and become correction-injection retrieval candidates. Existing flags surfaced as badge counts per section.
...
**Client-side sort + filters on the queue** (added 2026-05-10, Phase A; extended 2026-05-17): every column header in `QueueTab` cycles asc/desc on click. Fuzzy multi-token name search on Rep + Advisor, mirroring CoachingUsage's helper. Rep role filter (any/SGA/SGM) renders for admin/manager only (SGM/SGA already see only their own evals). **Stage filter** (2026-05-17): multi-select dropdown with all 9 stages, all selected by default. Stage data resolved via existing BQ `resolveAdvisorNames` round-trip — `sfdc_what_id` from `call_notes` passed as `whatIds`, `currentStage` from `whatIdToInfo` mapped onto each row as `opp_stage`. Rows without a linked opportunity (`opp_stage = null`) pass through all stage filters (only rows with a matching stage are excluded). **Pod filter** (2026-05-17): multi-select dropdown, manager/admin only. Pod data from `coaching_team_members` + `coaching_teams` LEFT JOINed in the Neon queue SQL (no extra round-trip). Pods fetched from `/api/call-intelligence/insights/pods`. `EvaluationQueueRow` extended with `pod_id`, `pod_name`, `opp_stage`.
...
| `/api/call-intelligence/evaluations/[id]/feedback` | POST | bridge | Manager/admin flag incorrect AI evaluation content. Writes to `ai_feedback` (pending_review); rows become correction-injection retrieval candidates. No OCC — flagging is advisory, not a mutation. |
...
| `/api/call-intelligence/opportunities` | GET | BQ + direct (coachingDb) | Opportunities tab list. BQ resolves all recruiting Opps with Lead/Contact identity tuples; Neon counts threaded calls per Opp via UNNEST. Filters: stage[], owner[], hasLikelyUnlinked. All allowed roles get full all-rep visibility; SGM/SGA roles are owner-scoped (only see their own opportunities). |
| `/api/call-intelligence/opportunities/[opportunityId]` | GET | BQ + direct (coachingDb) | Opportunity detail: header (BQ), threaded call timeline (Neon), stage-at-time-of-call badges (BQ OFH). SFDC Id validation (006-prefix). All allowed roles get full call visibility (no rep filtering). |
| `/api/call-intelligence/opportunities/[opportunityId]/ai-summary` | GET, POST | BQ + direct (coachingDb) + Claude API | AI-generated deal summary with 5 categories: pain points, competitors, next steps, compensation, advisor concerns. GET auto-generates on cache miss (24h TTL + hash). POST forces regeneration. Cached in `opportunity_ai_summaries` table (sales-coaching Neon). Uses Claude Sonnet for synthesis. All allowed roles get full call visibility. |
| `/api/call-intelligence/opportunities/[opportunityId]/chat` | GET, POST | direct (coachingDb) + Claude API + Vertex AI REST | Streaming chat with Claude about opportunity calls. GET loads/creates thread + messages, detects new calls via hash comparison. POST streams SSE response (max_tokens 8192) grounded in call summaries and hybrid-retrieved KB chunks: primary path is pgvector cosine search (Vertex AI text-embedding-004, distance threshold 0.65, topK=8) supplemented by a keyword-based fallback (`searchKbChunksByKeyword`, topK=25) that matches `doc_id`/`body_text` against detected topic signals (objection, comp, tech, transition, compliance, equity) — ensures retrieval even when embeddings fail or cosine distance is borderline, and captures entire playbook folders (e.g. all 20 objection chunks). Results from both paths are deduplicated before system-prompt injection. System prompt includes explicit "Objection Handling & Knowledge Base Usage" instructions requiring Claude to ground objection-related responses in KB playbooks with citations, and a "Writing Style" section enforcing concise bullet-point format (no multi-paragraph prose, 2-sentence max per bullet, verbatim quotes, compact tables for data, 2-3 screens max). System prompt includes Advisor Persona Framework (Salesperson / Investment Guru / Life Planner classification from call content + ICP KB context), Personal Rapport & Emotional Intelligence instructions (surfaces personal details from calls for rapport building), and Follow-Up Questions (Claude appends 2-3 contextual follow-ups in a delimited `---\n**Suggested follow-ups:**` format; client strips and renders as clickable chips). A supplemental persona-specific KB search always runs alongside the user-query search to ensure ICP/persona content is available; results are deduplicated before injection. Per-user threads in `opportunity_chat_threads`/`opportunity_chat_messages` (sales-coaching Neon). All allowed roles get full call visibility. |
...
| File | Purpose |
|---|---|
| `src/app/dashboard/call-intelligence/CallIntelligenceClient.tsx` | Tab strip with role-gated rendering. Tabs: Queue, Opportunities, Insights, Rubrics, Usage, **Tutorials**, My settings, Admin: Users, Admin: Content Refinements, Cost Analysis |
...
| `src/app/dashboard/call-intelligence/tabs/TutorialsTab.tsx` | Role-filtered video tutorials with embedded Google Drive players. SGAs see Kixie workflow video; SGMs see Granola notes, record linking, and opportunity/chat videos; managers/admins see all grouped by role section. Accordion cards expand to show description + 16:9 iframe player (`drive.google.com/file/d/{id}/preview`). Deep-linkable via `?tab=tutorials`. |
...
| `src/lib/queries/call-intelligence-evaluations.ts` | `getRepIdByEmail`, `getEvaluationsForManager`, `getEvaluationDetail`, `getTranscriptComments`, `getKbChunksByIds`, `getAiFeedbackForEval` |
