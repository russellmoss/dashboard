...
# Deploy to Cloud Run — image only, secrets/env vars are configured on the service
gcloud run deploy "${SERVICE_NAME}" \
  --project="${PROJECT_ID}" \
  --image="gcr.io/${PROJECT_ID}/${SERVICE_NAME}:latest" \
  --region="${REGION}"

# Clean up copied file (gitignored, but keep it tidy)
rm -f "${SCRIPT_DIR}/schema-config.yaml"
