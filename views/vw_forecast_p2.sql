...
-- Cohorts for rate estimation: Jun 2025 - Dec 2025 (resolved SQOs only: Joined + Closed Lost)
...
WITH historical_cohort AS (
  -- Resolved SQOs from Jun-Dec 2025 (Joined + Closed Lost only)
  -- Excludes open deals to avoid deflating rates with unresolved pipeline
  SELECT
    StageName,
...
historical_rates AS (
  ...
  SELECT
    ...
    -- Volume-weighted avg days (only from deals with both timestamps)
    SAFE_DIVIDE(
