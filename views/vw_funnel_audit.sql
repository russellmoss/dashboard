...
-- Full stage history audit trail for all opps (no date filter — filtered at query time)
...
WITH base AS (
  ...
  WHERE Full_Opportunity_ID__c IS NOT NULL
    ...
    -- No date filter here — applied at query time based on window selection
),
...
SELECT
  ...
  -- Conversion rate flags: 1/0 for numerator/denominator at each stage transition
  -- Only populated for resolved SQOs (SQO_raw='Yes' AND Joined/Closed Lost)
  -- SUM the columns directly — no additional filtering needed
  CASE WHEN eff_joined_ts IS NOT NULL AND StageName != 'Closed Lost' THEN 1 ELSE 0 END AS is_joined_flag,

  -- SQO → SP: denom = resolved SQO, numer = reached SP or beyond
  CASE WHEN SQO_raw = 'Yes' AND StageName IN ('Joined', 'Closed Lost')
       THEN 1 ELSE 0 END AS SP_Denominator,
  CASE WHEN SQO_raw = 'Yes' AND StageName IN ('Joined', 'Closed Lost')
            AND (eff_sp_ts IS NOT NULL OR eff_neg_ts IS NOT NULL OR eff_signed_ts IS NOT NULL
                 OR (eff_joined_ts IS NOT NULL AND StageName != 'Closed Lost'))
       THEN 1 ELSE 0 END AS SP_Numerator,

  -- SP → Neg: denom = resolved SQO that reached SP+, numer = reached Neg+
  CASE WHEN SQO_raw = 'Yes' AND StageName IN ('Joined', 'Closed Lost')
            AND (eff_sp_ts IS NOT NULL OR eff_neg_ts IS NOT NULL OR eff_signed_ts IS NOT NULL
                 OR (eff_joined_ts IS NOT NULL AND StageName != 'Closed Lost'))
       THEN 1 ELSE 0 END AS Neg_Denominator,
  CASE WHEN SQO_raw = 'Yes' AND StageName IN ('Joined', 'Closed Lost')
            AND (eff_neg_ts IS NOT NULL OR eff_signed_ts IS NOT NULL
                 OR (eff_joined_ts IS NOT NULL AND StageName != 'Closed Lost'))
       THEN 1 ELSE 0 END AS Neg_Numerator,

  -- Neg → Signed: denom = resolved SQO that reached Neg+, numer = reached Signed+
  CASE WHEN SQO_raw = 'Yes' AND StageName IN ('Joined', 'Closed Lost')
            AND (eff_neg_ts IS NOT NULL OR eff_signed_ts IS NOT NULL
                 OR (eff_joined_ts IS NOT NULL AND StageName != 'Closed Lost'))
       THEN 1 ELSE 0 END AS Signed_Denominator,
  CASE WHEN SQO_raw = 'Yes' AND StageName IN ('Joined', 'Closed Lost')
            AND (eff_signed_ts IS NOT NULL
                 OR (eff_joined_ts IS NOT NULL AND StageName != 'Closed Lost'))
       THEN 1 ELSE 0 END AS Signed_Numerator,

  -- Signed → Joined: denom = resolved SQO that reached Signed+, numer = joined
  CASE WHEN SQO_raw = 'Yes' AND StageName IN ('Joined', 'Closed Lost')
            AND (eff_signed_ts IS NOT NULL
                 OR (eff_joined_ts IS NOT NULL AND StageName != 'Closed Lost'))
       THEN 1 ELSE 0 END AS Joined_Denominator,
  CASE WHEN SQO_raw = 'Yes' AND StageName IN ('Joined', 'Closed Lost')
            AND eff_joined_ts IS NOT NULL AND StageName != 'Closed Lost'
       THEN 1 ELSE 0 END AS Joined_Numerator,
