...
async function run() {
  const [rows] = await bq.query({ query: `SELECT '180d' AS win, COUNT(*) AS n, ROUND(AVG(COALESCE(Underwritten_AUM__c, Amount)) / 1e6, 1) AS mean_aum_m
...
UNION ALL
SELECT '1yr' AS win, COUNT(*) AS n, ROUND(AVG(COALESCE(Underwritten_AUM__c, Amount)) / 1e6, 1) AS mean_aum_m
FROM \`savvy-gtm-analytics.Tableau_Views.vw_funnel_master\`
WHERE SQO_raw = 'Yes' AND is_primary_opp_record = 1 AND recordtypeid = '012Dn000000mrO3IAI'
  AND StageName = 'Joined' AND advisor_join_date__c IS NOT NULL
  AND COALESCE(Underwritten_AUM__c, Amount) > 0
...
UNION ALL
SELECT '2yr' AS win, COUNT(*) AS n, ROUND(AVG(COALESCE(Underwritten_AUM__c, Amount)) / 1e6, 1) AS mean_aum_m
FROM \`savvy-gtm-analytics.Tableau_Views.vw_funnel_master\`
WHERE SQO_raw = 'Yes' AND is_primary_opp_record = 1 AND recordtypeid = '012Dn000000mrO3IAI'
  AND StageName = 'Joined' AND advisor_join_date__c IS NOT NULL
  AND COALESCE(Underwritten_AUM__c, Amount) > 0
...
UNION ALL
SELECT 'all-time' AS win, COUNT(*) AS n, ROUND(AVG(COALESCE(Underwritten_AUM__c, Amount)) / 1e6, 1) AS mean_aum_m
FROM \`savvy-gtm-analytics.Tableau_Views.vw_funnel_master\`
WHERE SQO_raw = 'Yes' AND is_primary_opp_record = 1 AND recordtypeid = '012Dn000000mrO3IAI'
  AND StageName = 'Joined' AND advisor_join_date__c IS NOT NULL
  AND COALESCE(Underwritten_AUM__c, Amount) > 0` });
